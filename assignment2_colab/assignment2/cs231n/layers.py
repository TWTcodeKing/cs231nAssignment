...
def affine_forward(x, w, b):
    ...
    D=1
    indies=np.array(x.shape)
    for i in range(1,len(indies)):
        D*=indies[i]
    temp=np.reshape(x,(np.array(x.shape)[0],D))
    out=np.dot(temp,w)+b
    pass
...
def batchnorm_backward(dout, cache):
    ...
    N,D=x.shape
    dx_mean=dout*gamma
    dvar=dx_mean*(x-sample_mean)*(-0.5)*(sample_var+eps)**(-1.5)
    dvar=np.sum(dvar,axis=0,keepdims=True)
    
    dmean=np.sum(-dx_mean/np.sqrt(sample_var+eps),axis=0,keepdims=True)
    dmean+=dvar*np.sum(-2*(x-sample_mean)/N,axis=0,keepdims=True)
    
    dx=dx_mean/np.sqrt(sample_var+eps)
...
def layernorm_forward(x, gamma, beta, ln_param):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    out=np.zeros(x.shape)
    params_mean = np.mean(x, axis = 0)
    params_var = np.var(x, axis = 0)
    params_hat = (x - params_mean) / np.sqrt(params_var + eps)
    out = gamma * params_hat + beta
    cache=(x,params_hat,gamma,beta,eps,params_mean,params_var)
    pass
...
def layernorm_backward(dout, cache):
    ...
    ###########################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    x,params_hat,gamma,beta,eps,params_mean,params_var=cache
    N,D=x.shape
    dparams_mean=dout*gamma
    dvar=dparams_mean*(x-params_mean)*(-0.5)*(params_var+eps)**(-1.5)
    dvar=np.sum(dvar,axis=0,keepdims=True)
    
    dmean=np.sum(-dparams_mean/np.sqrt(params_var+eps),axis=0,keepdims=True)
    dmean+=dvar*np.sum(-2*(x-params_mean)/N,axis=0,keepdims=True)
    
    dx=dparams_mean/np.sqrt(params_var+eps)
    dx+=dvar*2*(x-params_mean)/N
    dx+=dmean/N
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    N,C_x,H,W=x.shape
    F,C_conv,HH,WW=w.shape
    H_conv = int(1 + (H + 2 * pad - HH) / stride)
    W_conv = int(1 + (W + 2 * pad - WW) / stride)
    out=np.ones(shape=(N,F,H_conv,W_conv))
    x_padding=np.zeros(shape=(N,C_x,H+2*pad,W+2*pad))
    ###########################################################################
    ...
    for FilterNum in range(F):
        for height in range(H_conv):
            for width in range(W_conv):
                out[np.arange(N),FilterNum,height,width]=np.sum(w[FilterNum,:,:,:]*x_padding[np.arange(N),:,height*stride:height*stride+HH,width*stride:width*stride+WW],axis=(1,2,3))+b[FilterNum]
    pass
...
def max_pool_forward_naive(x, pool_param):
    ...
    for height in range(H_pool):
        for width in range(W_pool):         out[:,:,height,width]=np.max(x[:,:,height*stride:height*stride+pool_height,width*stride:width*stride+pool_width],axis=(2,3))
    pass
